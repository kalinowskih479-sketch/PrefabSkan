...
public class OcrInlineManholeFieldsRegressionTests
{
    ...
    public async Task Ocr_Profile_Recovers_Inline_Dn_Height_Type_And_Crown_When_Spatial_Window_Is_Fragmented()
    {
        ...
        var page = new PageText { PageNumber = 1, Text = "[[PREFABSCAN_DRAWING:PROFIL]] D7 DN1200 H=1.33 betonowa wlaz zeliwny", ExtractionEngine = "OCR/Tesseract tiled" };
        ...
        Assert.Equal(1200, d7.DiameterMm); Assert.Equal(1.33, d7.HeightM); Assert.Equal("betonowa", d7.Type); Assert.Equal("właz żeliwny", d7.Crown);
    }

    [Fact]
    public async Task Ocr_Profile_Recovers_D7_And_S7_With_Observed_Noise_Before_Dn()
    {
        var parser = new OcrResilientProjectParser();
        var page = new PageText { PageNumber = 1, Text = "[[PREFABSCAN_DRAWING:PROFIL]] D/ Pa DN1200 PA H=1.33 betonov wlaz zeliwny 57 DN1200 H=1.14 betonov wlaz zeliwny", ExtractionEngine = "OCR/Tesseract tiled" };
        var result = await parser.ParseAsync(new[] { page });
